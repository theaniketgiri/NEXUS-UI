...
export function validateEnvironment(): void {
  ...
  if (heapLimitGB < 2) {
    console.warn(
      chalk.yellow(
        `Warning: Available heap memory (${heapLimitGB}GB) might be insufficient.\n` +
  ...
  try {
    ...
    throw new CLIError(
      ...
      'Please install it with: npm install -g typescript',
      'TYPESCRIPT_NOT_FOUND'
    );
  }
}

export async function validateFileSystem(): Promise<void> {
  ...
  for (const dir of requiredDirs) {
    try {
      ...
      throw new CLIError(
        ...
        'Please ensure you are in the correct project directory.',
        'DIRECTORY_NOT_FOUND'
      );
    }
  }
  ...
  try {
    ...
    throw new CLIError(
      ...
      'Please ensure you have write access to the project directory.',
  ...
  try {
    ...
    if (availableGB < 1) {
      throw new CLIError(
        `Insufficient disk space. Only ${availableGB}GB available.\n` +
        'Please ensure at least 1GB of free space.',
    ...
    console.warn(
      chalk.yellow(
        'Warning: Unable to check available disk space.\n' +
